...
export function PricingCard({
	...
	return (
		<Card
			variant="gradient"
			className={`lg:w-[25rem] w-full h-fit ${colorType == 1 ? "bg-gray-900" : "white"} border border-[#c4c4c4]/40`}
		>
			<div
				className="absolute inset-0 bg-cover bg-no-repeat opacity-20"
				style={{
					backgroundImage: `url(${colorType == 1 ? patterns : patternsDark})`,
				}}
			/>
			...
			<CardBody className="relative z-10 text-center transition-all">
				...
				<Accordion
					open={open === 1}
					className="my-6 lg:hidden w-full"
					icon={<Icon id={1} open={open} />}
				>
					<AccordionHeader
						onClick={() => handleOpen(1)}
						className={`text-center flex items-center justify-center ${colorType == 1 ? "text-white" : "text-gray-900"}`}
					>
						Veja os benefícios
					</AccordionHeader>
					<AccordionBody>
				...
				<Button
					variant="filled"
					className="bg-[#7ED956] text-black"
					id="contrate"
					aria-label="contrate"
					onClick={() => openInNewTab(checkout)}
				>
					Contrate agora
				</Button>
			</CardBody>
